...
function isIn2DMode() {
    ...
    var in2d = (EXTERNALLY_MANAGED_2D_MINOR_MODE &&
        (Reticle.pointingAtSystemOverlay || Overlays.getOverlayAtPoint(Reticle.position)));
    //if (!in2d) {
        printd("isIn2DMode EXTERNALLY_MANAGED_2D_MINOR_MODE: " + EXTERNALLY_MANAGED_2D_MINOR_MODE + " Reticle.pointingAtSystemOverlay: " + Reticle.pointingAtSystemOverlay +
            " Overlays.getOverlayAtPoint(Reticle.position): " + Overlays.getOverlayAtPoint(Reticle.position) + " Reticle.position: " + JSON.stringify(Reticle.position));
    //}
    return in2d;
}
...
function MyController(hand) {
    ...
    this.ignoreInput = function() {
        ...
        return false;
        var logHand = (this.hand==RIGHT_HAND);
        var is2d = isIn2DMode();
        var igno = (-1 !== suppressedIn2D.indexOf(this.state)) && is2d;
        //if (!igno) {
            if (logHand) printd("ignoreInput result: " + igno + " suppressedIn2D.indexOf(this.state):" + (suppressedIn2D.indexOf(this.state)) + " isIn2DMode: "  + is2d + " suppressedIn2D: " + JSON.stringify(suppressedIn2D) + " state: " + this.state);    
        //}
        return igno;
    };
    ...
    this.update = function(deltaTime, timestamp) {
        var logHand = (this.hand==RIGHT_HAND);
        if (logHand) printd("update myController START");
        this.updateSmoothedTrigger();
        if (logHand) printd("update myController new trigger value " + this.triggerValue);
        if (this.ignoreInput()) {
            if (logHand) printd("update myController ignoreInput OUT return");
            this.turnOffVisualizations();
            return;
        }
        if (logHand) printd("update analyse hand(" + this.hand + ") state machine state: " + this.state + " (" + JSON.stringify(CONTROLLER_STATE_MACHINE));
        if (CONTROLLER_STATE_MACHINE[this.state]) {
            ...
            if (logHand) printd("update updateMethod " + JSON.stringify(updateMethod) + " name: " + updateMethodName);
            if (updateMethod) {
                if (logHand) printd("update calling updateMethod " + JSON.stringify(updateMethod) + " name: " + updateMethodName);
                updateMethod.call(this, deltaTime, timestamp);
                if (logHand) printd("update updateMethod (name: " + updateMethodName + " called");
            } else {
                if (logHand) printd("WARNING: could not find updateMethod for state " + stateToName(this.state));
            }
        } else {
            if (logHand) printd("WARNING: could not find state " + this.state + " in state machine");
        }
        if (logHand) printd("update myController END");
    };
    ...
    this.setState = function(newState, reason) {
        var logHand = (this.hand==RIGHT_HAND);
        if (logHand) printd("setState to: " + newState + " because: " + reason + " hand(" + this.hand);
        ...
        if (WANT_DEBUG || WANT_DEBUG_STATE) {
            ...
            if (logHand) print("STATE (" + this.hand + "): " + newStateName + " <-- " + oldStateName + ", reason = " + reason);
        }
        ...
        if (CONTROLLER_STATE_MACHINE[newState]) {
            ...
            if (logHand) printd("WARNING: could not find newState " + newState + " in state machine");
        }
        if (logHand) printd("new state: " + this.state + " hand(" + this.hand);
    };
    ...
    this.updateSmoothedTrigger = function() {
        var logHand = (this.hand == RIGHT_HAND);
        var logg = this.rawTriggerValue != this.triggerValue || this.rawTriggerValue > 0;
        var oldv = this.triggerValue;
        var triggerValue = this.rawTriggerValue;
        // smooth out trigger value
        this.triggerValue = (this.triggerValue * TRIGGER_SMOOTH_RATIO) +
            (triggerValue * (1.0 - TRIGGER_SMOOTH_RATIO));
        if (logg) {
            if (logHand) printd("updateSmoothedTrigger " + oldv + " --to--> " + this.triggerValue);
        }
    };
    ...
    this.off = function(deltaTime, timestamp) {
        var logHand = (this.hand == RIGHT_HAND);
        if (logHand) printd("off START");
        if (this.triggerSmoothedReleased() && this.secondaryReleased()) {
            this.waitForTriggerRelease = false;
        }
        if (logHand) printd("off values waitForTriggerRelease["+this.waitForTriggerRelease+"] triggerSmoothedSqueezed[" + this.triggerSmoothedSqueezed()
               + "] secondarySqueezed[" + this.secondarySqueezed() + "]");
        if (!this.waitForTriggerRelease && (this.triggerSmoothedSqueezed() || this.secondarySqueezed())) {
            if (logHand) printd("off about to setState SEARCHING");
            this.lastPickTime = 0;
        ...
        if (logHand) printd("off continues, no setState SEARCHING done");
    ...
    this.search = function(deltaTime, timestamp) {
        var logHand = (this.hand == RIGHT_HAND);
        if (logHand) printd("search START");
        var _this = this;
        var name;
        ...
        if ((this.triggerSmoothedReleased() && this.secondaryReleased())) {
            ...
            if (logHand) printd("search trigger released, returning END");
            return;
        }

        var controllerLocation = getControllerWorldLocation(this.handToController(), true);
        printd("search controllerLocation " + JSON.stringify(controllerLocation));
        var handPosition = controllerLocation.position;

        var rayPickInfo = this.calcRayPickInfo(this.hand);
        if (logHand) printd("search rayPickInfo " + JSON.stringify(rayPickInfo));
        ...
        if (logHand) printd("search candidateHotSpotEntities " + JSON.stringify(candidateHotSpotEntities));
        ...
        if (potentialEquipHotspot) {
            if ((this.triggerSmoothedGrab() || this.secondarySqueezed()) && holdEnabled) {
                ...
                if (logHand) printd("search switch to equipping, returning END");
                return;
            }
        }

        var candidateEntities = Entities.findEntities(handPosition, NEAR_GRAB_RADIUS);
        if (logHand) printd("search candidateEntities " + JSON.stringify(candidateEntities));
        var grabbableEntities = candidateEntities.filter(function(entity) {
            return _this.entityIsNearGrabbable(entity, handPosition, NEAR_GRAB_MAX_DISTANCE);
        });
        if (logHand) printd("search grabbableEntities " + JSON.stringify(grabbableEntities));
        if (rayPickInfo.entityID) {
            this.intersectionDistance = rayPickInfo.distance;
        ...
        var entity;
        if (grabbableEntities.length > 0) {
            if (logHand) printd("search grabbableEntities.length > 0 ");
            // sort by distance
            ...
            if (logHand) printd("search GRABBED ENTITY!! " + JSON.stringify(this.grabbedEntity) + " END");
            if (this.entityWantsTrigger(entity)) {
                if (this.triggerSmoothedGrab()) {
                    this.setState(STATE_NEAR_TRIGGER, "near trigger '" + name + "'");
                    if (logHand) printd("search switch near trigger, returning END");
                    return;
                } else {
                ...
                if ((this.triggerSmoothedGrab() || this.secondarySqueezed()) && nearGrabEnabled) {
                    ...
                    if (logHand) printd("search switch to near grab, returning END");
                    return;
                } else {
            ...
            if (logHand) printd("search grabbableEntities.length <= 0 NOTHING TO GRAB?");
        }

        var pointerEvent;
        if (rayPickInfo.entityID && Entities.wantsHandControllerPointerEvents(rayPickInfo.entityID)) {
            if (logHand) printd("search ayPickInfo.entityID && Entities.wantsHandControllerPointerEvents(rayPickInfo.entityID)");
            ...
            if (this.triggerSmoothedGrab() && !isEditing()) {
                ...
                if (logHand) printd("search switch to begin touching entity, returning END");
                return;
            }
        ...
        if (rayPickInfo.entityID) {
            ...
            if (this.entityWantsTrigger(entity)) {
                if (this.triggerSmoothedGrab()) {
                    ...
                    if (logHand) printd("search switch to far trigger, returning END");
                    return;
                } else {
                ...
                if (this.triggerSmoothedGrab() && !isEditing() && farGrabEnabled && farSearching) {
                    ...
                    if (logHand) printd("search switch to distance hold, returning END");
                    return;
                } else {
        ...
        var overlay;
        if (logHand) printd("search before if rayPickInfo.overlayID");
        if (rayPickInfo.overlayID) {
            if (logHand) printd("search rayPickInfo.overlayID");
            ...
            if (this.triggerSmoothedGrab() && !isEditing()) {
                ...
                if (logHand) printd("search switch to begin touching overlay, returning END");
                return;
            }

        } else if (this.hoverOverlay) {
            if (logHand) printd("search this.hoverOverlay");
            pointerEvent = {
                type: "Move",
                id: HARDWARE_MOUSE_ID
            };
        ...
        equipHotspotBuddy.updateHotspots(nearEquipHotspots, timestamp);
        if (potentialEquipHotspot) {
            if (logHand) printd("search potentialEquipHotspot");
            equipHotspotBuddy.highlightHotspot(potentialEquipHotspot);
        }

        if (farGrabEnabled && farSearching) {
            if (logHand) printd("search this.searchIndicatorOn");
            this.searchIndicatorOn(rayPickInfo.searchRay);
        }
        Reticle.setVisible(false);
        if (logHand) printd("search END");
    };
