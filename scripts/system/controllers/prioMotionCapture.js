...
function PrioVRAvatar() {
    ...
    if (Controller.Hardware.PrioVR) {
        Script.setTimeout(function(){self.activate()}, 5000);   
    } else {
        this.deactivate();
    }
}

PrioVRAvatar.prototype.shutdown = function () {
    this.deactivate();
...
PrioVRAvatar.prototype.hardwareChanged = function () {
    if (Controller.Hardware.PrioVR) {
        
        Script.setTimeout(this.activate, 5000);     
...
PrioVRAvatar.prototype.activate = function () {   
...
PrioVRAvatar.prototype.deactivate = function () {
    if (this._active) {
        var self = this;
...
var TRACKED_OBJECT_POSES = [
    "LeftFoot", "RightFoot", "Hips", "Spine1",
    "LeftShoulder", "LeftArm", "LeftForeArm", "LeftHand",
    "RightShoulder", "RightArm", "RightForeArm", "RightHand",
    "LeftUpLeg", "Neck", "RightUpLeg", "Head"
];
...
var TRIGGER_MAPPING_NAME = "com.highfidelity.prioMotionCapture.triggers";
...
triggerMapping.from([Controller.Hardware.Keyboard.Space]).peek().to(function (value) {
    leftTriggerPressed = (value !== 0) ? true : false;
	rightTriggerPressed = (value !== 0) ? true : false;
});
...
var CONTROLLER_MAPPING_NAME = "com.highfidelity.prioMotionCapture.controller";
...
function calibrate() {
    ...
    var poses = [];
    if (Controller.Hardware.PrioVR) {
		print("WAB: PrioVR found.");
        TRACKED_OBJECT_POSES.forEach(function (key) {
            var channel = Controller.Hardware.PrioVR[key];
    ...
    if (poses.length >= 2) {
		...
		//WAB: reduce each translation by a factor of 100... for funsies... :)
	/*	for(var jimjamz=0; jimjamz<poses.length; jimjamz++){
			poses[jimjamz].pose.translation.x /=100;
			poses[jimjamz].pose.translation.y /=100;
			poses[jimjamz].pose.translation.z /=100;
		}
...
var priovrAvatar = new PrioVRAvatar();

function updateCallback(deltaTime) {
    priovrAvatar.update(deltaTime);
}
//dumpHardwareMapping();
